...
module.exports = {
    ...
    async handleSelectMenuInteraction(interaction) {
        try {
            ...
            if (customId === 'economy_config_menu') {
                ...
                await this.handleActionConfigSelection(interaction, value);
            } else if (customId === 'economy_action_settings') {
                await this.handleActionSettings(interaction, value);
    ...
    // === MÉTHODES MANQUANTES POUR CONFIGURATION ACTIONS ===

    async handleActionConfigSelection(interaction, action) {
        try {
            console.log(`HandleActionConfigSelection called with action: ${action}`);
            await interaction.deferUpdate();

            const embed = new EmbedBuilder()
                .setColor('#4CAF50')
                .setTitle(`⚙️ Configuration ${this.getActionName(action)}`)
                .setDescription(`Sélectionnez l'aspect à configurer pour l'action **${this.getActionName(action)}**`)
                .addFields([
                    {
                        name: '💰 Récompense',
                        value: 'Configurer les gains d\'argent',
                        inline: true
                    },
                    {
                        name: '⏰ Cooldown',
                        value: 'Définir le temps d\'attente',
                        inline: true
                    },
                    {
                        name: '⚖️ Karma',
                        value: 'Configurer l\'impact karma',
                        inline: true
                    }
                ]);
            ...
            const components = [
                ...
                new ActionRowBuilder()
                    .addComponents(
                        new StringSelectMenuBuilder()
                            .setCustomId('economy_action_settings')
                            .setPlaceholder(`🎯 Configurer ${this.getActionName(action)}`)
                            .addOptions([
                                {
                                    label: 'Récompense/Gain',
                                    description: `Configurer les gains d'argent pour ${this.getActionName(action)}`,
                                    value: `reward_${action}`,
                                    emoji: '💰'
                                },
                                {
                                    label: 'Cooldown',
                                    description: `Définir le temps d'attente pour ${this.getActionName(action)}`,
                                    value: `cooldown_${action}`,
                                    emoji: '⏰'
                                },
                                {
                                    label: 'Karma Bon',
                                    description: `Configurer l'impact karma positif`,
                                    value: `karma_good_${action}`,
                                    emoji: '😇'
                                },
                                {
                                    label: 'Karma Mauvais',
                                    description: `Configurer l'impact karma négatif`,
                                    value: `karma_bad_${action}`,
                                    emoji: '😈'
                                }
                            ])
                    )
            ...
            console.error('Erreur handleActionConfigSelection:', error);
        }
    },

    async handleActionSettings(interaction, value) {
        try {
            console.log(`HandleActionSetting called with value: ${value}`);
            
            const [setting, action] = value.split('_').slice(-2);
            const settingType = value.replace(`_${action}`, '');
            
            if (settingType === 'reward') {
                await this.showRewardModal(interaction, action);
            } else if (settingType === 'cooldown') {
                await this.showCooldownModal(interaction, action);
            } else if (settingType === 'karma_good') {
                await this.showKarmaGoodModal(interaction, action);
            } else if (settingType === 'karma_bad') {
                await this.showKarmaBadModal(interaction, action);
            }
        } catch (error) {
            console.error('Erreur handleActionSettings:', error);
        }
    },

    async showRewardModal(interaction, action) {
        try {
            const modal = new ModalBuilder()
                .setCustomId(`economy_reward_${action}`)
                .setTitle(`💰 Configurer Gains - ${this.getActionName(action)}`);

            const rewardInput = new TextInputBuilder()
                .setCustomId('reward_amount')
                .setLabel('Montant de la récompense (€)')
                .setStyle(TextInputStyle.Short)
    ...
    async showCooldownModal(interaction, action) {
        try {
            const modal = new ModalBuilder()
                .setCustomId(`economy_cooldown_${action}`)
                .setTitle(`⏰ Configurer Cooldown - ${this.getActionName(action)}`);
